...
current_trails %>%
  summarise(min = min(length), 
            q1 = quantile(length, 1/4), 
            median = median(length), 
            q3 = quantile(length, 3/4), 
            max = max(length),
            iqr = quantile(length, 3/4) - quantile(length, 1/4), 
            mean = mean(length), 
            std_dev = sd(length)
            )
```
...
```{r eval = F}
IQR <- quantile(current_trails$length, 3/4) - quantile(current_trails$length, 1/4)
q3 <- quantile(current_trails$length, 3/4)
q1 <- quantile(current_trails$length, 1/4)
Upper<-q3+1.5*IQR
Lower<-q1-1.5*IQR
new_trails <- subset(current_trails, length <= 5 & length <=Upper & length>=Lower) 
glimpse(new_trails)
````
...
```{r eval = F}
current_class <- trails %>%
  filter(status == "Existing", !is.na(class))
ggplot(data = current_class) +
  geom_histogram(mapping = aes(x = class), binwidth = 0.5)
current_class %>%
  summarise(min = min(class), 
            q1 = quantile(class, 1/4), 
            median = median(class), 
            q3 = quantile(class, 3/4), 
            max = max(class),
            iqr = quantile(class, 3/4) - quantile(class, 1/4), 
            mean = mean(class), 
            std_dev = sd(class)
            )
```
...
```{r eval=FALSE}
current_class <- trails %>% filter(class !=3) %>% 
  mutate(class = if_else(is.na(class),0,class)) 
glimpse(current_class)
ggplot(data = current_class) +
  geom_histogram(mapping = aes(x = class), binwidth = 0.5)
...
```{r eval=FALSE}
ggplot(data = current_class, mapping = aes(x = class, y = length)) +
  geom_boxplot()
```
...
t.test(length ~ class, data = current_class, 
